...
public class E04_SetCover {
    ...
    public static List<int[]> chooseSets(List<int[]> sets, int[] universe) {
        ...
        int[] originalsValues = Arrays.copyOf(universe,universe.length);

        while (originalsValues.length > 0 && initialSets.size() > 0) {
            int maxIntersectionSize = 0;
            int maxSetIndex = -1;
            for (int i = 0; i < initialSets.size(); i++) {

                int currentIntersectionSize = getIntersectionSize(initialSets.get(i), originalsValues);
            ...
            originalsValues = removeValues(originalsValues, selected);
            result.add(selected);
        }
